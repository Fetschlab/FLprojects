function [totalError, logOddsMap, posteriorProb_LatentState_GivenChoice, firstRaceLoss] = FP_Drugowitsch_Fitting_AllInOne(data, numOfImages, endTi, deltaT, bound, whichSolve, extraParamsName, params)% Bl, Bu, T0, sigma0, sensitivity)
...
%modelCoherences =  [-.512 -.256 -.128 -.064 -.032 0 .032 .064 .128 .256 .512]; %Should this be the coherences that i use for the model????
...
%             rt_Low_Temp(c) = sum(distribution_RTlow_model .* ti(1:300));
            ...
            % TEMP - CF
            probRightConditionedHighRT = probRightConditionedHighRT';
            probRightConditionedLowRT = probRightConditionedLowRT';
