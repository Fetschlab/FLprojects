...
* 10/08/2024
*
* FP_Drugowitsch.c - computes closed-form solution for 2D Fokker-Planck
* equation, to calculate confidence in a 2D bounded accumulator model (aka
* anticorrelated race).
*
...
*
% Included as a dependency for code provided for peer review of:
% M. Vivar-Lazo & C.R. Fetsch, Neural basis of concurrent deliberation
% toward a choice and degree of confidence (2024)
* 
* Intended to be compiled as a MEX file for MATLAB.
*/
...
long drugo(double* subInd, double delta, int xmeshSize, double* mu, double k, double totalTime, double dt, double* s0, double alpha, double** globalSigma, double urgencyMax, double urgencyTauHalf, double* xytFPFinal){
	...
	for (t = 0, r = 0; t < totalTime; t = t + dt, ++r){
		...
		if (urgencyMax != 0 && urgencyTauHalf != 0){
			additionalMu[0] = mu[0] + urgencyMax * (t / (t + urgencyTauHalf)); // + (urgencyMax * (t / (t + urgencyTauHalf)))/t
			additionalMu[1]  = mu[1] + urgencyMax * (t / (t + urgencyTauHalf));
		}
            ...
            additionalMu[0] = mu[0] + (urgencyMax / totalTime * .001); // should this be -
            additionalMu[1] = mu[1] + (urgencyMax / totalTime * .001);
		}
		else{
