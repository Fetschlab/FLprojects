...
% Uses FP_Drugowitsch.m, which is an implementation of the method in:
...
% plotBehav(parsedData,unique(data.cohs),conftask,RTtask,0);
% 
...
for i = 1:iterationsSearch
    ...
    [tempOutcomeParams_wTheta, errorLL] = fminsearchbnd(@(params0_wTheta) FP_Drugowitsch_Fitting_AllInOne(monkeyData, k, totalTime, deltaT, bound, whichSolve, extraParamsName, params0_wTheta), params0_wTheta, LB, UB, options); %FP_Drugowitsch_Fitting(monkeyData, rho, Bl, Bu, T0, sigma0, sensitivity)
    ...
    % Actally save
    save(['/Users/mvivarl1/Documents/MATLAB/Dataset_Miguels/genji_Fitting_k=' num2str(k) '_Joint_' char(datetime("today")) '.mat'], 'outcomeParamsMat_wTheta', 'allErrorLL_wTheta', 'gridDesign');    
%     save(['/Users/mvivarl1/Documents/MATLAB/Dataset_Miguels/genji_Fitting_k=' num2str(k) '_Hyperbolic_JointProb_' char(datetime("today")) '.mat'], 'outcomeParamsMat_wTheta', 'allErrorLL_wTheta', 'gridDesign');
...
% MAKE CODE FOR COMPUTING A LIKELIHOOD FOR PREDICTIONS AND NOT JUST FITS

%% Now that we have fits, use those to fit the data. 

[totalError, logOddMaps_FromFit] = FP_Drugowitsch_Fitting_AllInOne(monkeyData, k, totalTime, deltaT, bound, whichSolve, extraParamsName, outcomeParams);
...
% In the conditionals figure out this pattern and why this pattern exist
% and not its inverse:
%       Basically, when the P(High Conf) pattern decreases the sensitivity
%       seperation between P(Right|High) vs P(Right|Low) decreases. When
%       the P(High Conf) pattern increases the sensitivity seperation
%       increases. Now this is frustrating because of two reasons... 1) it
%       doesnt make sense, if I where likelier to make low betting outcomes
%       wouldn't I make more mistake on those outcomes given me a less
%       sensitiivty than if i was to not bet low as much? 2) Genji shows
%       this pattern where he bets low more... however he also has a wider
%       gap in sensitivity between P(Right|High) vs P(Right|Low) than the
%       model would predict, the model predicts very little difference in
%       sensivitity between P(Right|High) vs P(Right|Low).
%   Why does the model make this pattern of predictions? And can we reverse
%   that relationship? I am sure there is a simple way of doing this. 
...
nameOfModel{indexModel} = 'Hanzo 2D_Accumulator';
...
% Genji -LogLL: Correct=3.7684e+04 (bic=7.5437e+04), Incorrect=1.6283e+04 (bic=3.2626e+04)
% Hanzo -LogLL: Correct=2.8125e+04 (bic=5.6328e+04), Incorrect=8.9626e+03 (bic=1.7992e+04)
...
for i = 1:length(uniqIndVar)
    avgDepVar(i)    = nanmean(depVariable(uniqIndVar(i) == indVariable));
    N(i)            = nansum(uniqIndVar(i) == indVariable);
    stderr(i)       = nanstd(depVariable(uniqIndVar(i) == indVariable))/sqrt(N(i));
end
end