...
% Wrapper for fitting choice, RT, and wager data to "serial" model. 
%
...
asymmetrical = 1; % Only for monkey H
wagerOffset = 1; % Only for monkey H

numParams = 5 + useUrgency + asymmetrical + wagerOffset;
extraParams = 0;

options = optimset('Display','iter','TolX', 1, 'TolFun', 1, 'MaxIter', 10, 'MaxFunEvals', 10);
...
for i = 1:iterationsSearch
    ...
    [tempOutcomeParams, errorLL] = fminsearchbnd(@(params0) sequentialModel_Fitting(monkeyData, cohs, tEnd, howToCalculateError, params0, extraParamsName), params0, LB, UB, options);
...
[~, avgChoice, avgRT, avgConf, avgRightHigh, avgRightLow, avgRTHigh, avgRTLow, avgHigh_Corr, avgHigh_Inco] ...
    = sequentialModel_Fitting(data, modelCoherence, tEnd, howToCalculateError, params0, extraParamsName);
...
nameOfModel{indexModel} = 'Genji Sequential';
